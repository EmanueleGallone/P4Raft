...
def send_no_reply(message):
    sendp(message, iface='ve_A', count=1, return_packets=False, verbose=False)
...
class MyThread(Thread):
    ...
    def run(self):
        ...
        if args.command != COMMANDS['NewRequest']: # sending one packet per time
            sleep_time = 0.9
        ...
        while not self.stopped.wait(sleep_time):
            ...
            _time = time.time()
            print("sent packet; destinationID: {}; : time: {}".format(message[Raft].destinationID, _time))
            count += 0x1
...
class PacketGenerator(object):
    def __init__(self, max_num_of_packets, max_num_of_threads, command):
        ...
        self.total_time = 0
        self.socket = conf.L2socket('ve_A')
...
if __name__ == '__main__':
    ...
    if args.generator:
        gen = PacketGenerator(max_num_of_packets=200, max_num_of_threads=2, command=COMMANDS['NewRequest'])
        gen.start()
        #time.sleep(1)
        exit()
